...
def clustering(data , K, N):
    ...
    #On verifie le taux de réussite
    CX = np.zeros([128])
    CY = np.ones([128])
...
def choosegoodK (data):
    ...
    #On réalise une boucle ou nous allons noté l'intertie
    #et la silhouette à chauqe ittération
    for i in range(1, 7):
...
def CAH_complete_methode (data,K):
    #Clustering avec la methode CAH "complete"
    ...
    #Ajout la ligne horizontale de la coupe
